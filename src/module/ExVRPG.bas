...
#Const USELEGACY = 0
#Const HYDEBUG = 0 'm'' hyper debug mode flag, adding tons and tons of debug info

#If USELEGACY = 1 Then
Public Const curversion = "2.14 Legacy" 'm'' modified to know where we are
#ElseIf HYDEBUG = 1 Then
Public Const curversion = "2.14 -- hyper DEBUG" 'm'' debugmode!
#Else
...
#If HYDEBUG = 1 Then 'm''
dbmsg ("cStage.CreateAtSize 800, 600") 'm''
#End If 'm''
...
#If HYDEBUG = 1 Then 'm''
dbmsg ("backgr.CreateFromFile from Main()") 'm''
#End If 'm''
...
#If HYDEBUG = 1 Then 'm''
dbmsg ("makesprite digbody() from Main()") 'm''
#End If 'm''
...
#If HYDEBUG = 1 Then 'm''
dbmsg ("Form1.updatbody from Main()") 'm''
#End If 'm''
...
#If HYDEBUG = 1 Then 'm''
dbmsg ("Form1.Timer1.Enabled = True from Main()") 'm''
#End If 'm''
...
Do While plr.foodinbelly > 0
    For a = 1 To UBound(mon())
        If mon(a).instomach = -1 Then plrdigmon mon(a), a, 1, True 'm'' added true mean silent digestion
    Next a
    digestfood True 'm'' added true, mean silent digestion
Loop
...
#If HYDEBUG = 1 Then 'm''
dbmsg ("newchar getname and strip") 'm''
#End If 'm''
...
#If HYDEBUG = 1 Then 'm''
dbmsg ("newchar setup class skills") 'm''
#End If 'm''
...
#If HYDEBUG = 1 Then 'm''
dbmsg ("newchar Pickcombatskills.Show") 'm''
#End If 'm''

Pickcombatskills.Show 1

#If HYDEBUG = 1 Then 'm''
dbmsg ("newchar makesprite gbody") 'm''
#End If 'm''
...
#If HYDEBUG = 1 Then 'm''
dbmsg ("newchar Form1.updatbody") 'm''
#End If 'm''
...
#If HYDEBUG = 1 Then 'm''
dbmsg ("newchar Form1.updatinv") 'm''
#End If 'm''
...
Sub wingame()
Dim goldwin As Double 'm'' for money bound check
...
#If USELEGACY = 1 Then
'm'' End 'm'' such an abrupt ending may fuck the system
Debugger.Quitting 'm''
#Else
...
If rep = vbNo Then
    Debugger.Quitting 'm''
Else
    'm'' Revx_Z note : when ending, we should be rich.
    goldwin = (149000 * plr.level + Rnd * 1000) * (winlev + 1 + winlev) + plr.gp 'm'' a formula
    If goldwin < 1000000000# Then 'm'' bound check
        plr.gp = CLng(goldwin) 'm'' conversion
    Else 'm''
        plr.gp = 1000000000 + (Rnd * 6000000) 'm'' extreme gold, capped
    End If 'm''
End If
#End If
End Sub
...
Function digestfood(Optional Silent As Boolean = False) 'added the optional "silent" to avoid useless noises

#If USELEGACY <> 1 Then 'm''
    If Form1.menu_mod_cheat(2).Checked = True Then Exit Function 'm'' do not digest
#End If 'm''
...
If plr.foodinbelly = 0 Then
    Form1.updatbody
    playsound "fart57.wav"
    If Not Silent Then createobj "Shit", plr.X, plr.Y 'm'' added if not silent here
    gamemsg "Your " & getbelly("") & " has diligently " & getabsorb & "ed everything you have put in it."
End If
...
Function plrdigmon(ByRef mon As amonsterT, ByVal monnum As Integer, Optional force = 0, Optional Silent As Boolean = False) 'm'' added Silent
...
#If USELEGACY <> 1 Then 'm''
    If force = 0 And Form1.menu_mod_cheat(2).Checked = True Then Exit Function 'm'' do not digest
#End If 'm''

If turncount Mod 15 = 0 Or force > 0 Then
    If Not Silent Then playsound "stomach" & roll(4) & ".wav" 'm'' added if not silent
    ...
    If Not Silent Then playsound "burp" & roll(5) & ".wav" 'm'' added if not silent
    ...
    If Not Silent Then playsound "fart22.wav" 'm'' added if not silent
    Form1.updatbody
    Exit Function
    End If
...
Dim a As Long, b As Long 'm'' perfs
...
#Else
    #If HYDEBUG = 1 Then
        ...
        Dim fp As Integer 'm''
        fp = FreeFile 'm''
        Open "debug.log" For Append As #fp 'm''
        Print #fp, Date & " " & Timer, txt 'm''
        Close #fp 'm''
    #End If
#End If
